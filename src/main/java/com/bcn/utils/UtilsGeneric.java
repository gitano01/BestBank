...
//				System.out.println(resultSet.getMetaData().getColumnLabel(i).toLowerCase()+" >> ("+resultSet.getMetaData().getColumnType(i )+") == "+resultSet.getObject(i).toString());
				...
				case Types.DECIMAL:
					object.addProperty(resultSet.getMetaData().getColumnLabel(i).toLowerCase(),
							(resultSet.getObject(i) == null ? 0.0
									: new DecimalFormat("###,###,###,###.##")
											.parse(resultSet.getObject(i).toString())));
					break;
				case Types.INTEGER:
				case Types.BIGINT:
				case Types.SMALLINT:
				...
				case Types.BOOLEAN:
					object.addProperty(resultSet.getMetaData().getColumnLabel(i).toLowerCase(),
							Boolean.parseBoolean(resultSet.getObject(i).toString()));
					break;
	...
	public String generateCuenta() {
		String numero_cuenta = "";
		Random digito_verificador = new Random();
		long leftLimit = 1000000000L;
		long rightLimit = 9999999999L;
		long cuenta = 0L;

		cuenta = leftLimit + (long) (Math.random() * (rightLimit - leftLimit));
		numero_cuenta = "" + cuenta + "" + digito_verificador.nextInt((10 - 1) + 1);
